...
const LearningPage = () => {
  ...
  const [videoURLs, setVideoURLs] = useState<string[]>(location.state?.videoUrls || []);
  const [loading, setLoading] = useState(location.state?.videoUrls ? false : true);
  ...
  useEffect(() => {
    if (location.state?.videoUrls && location.state.videoUrls.length > 0) {
      return; // Skip Firebase fetch if we already have local URLs
    }
