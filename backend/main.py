...
if not firebase_initialized:
    try:
        ...
        bucket = storage.bucket()
        firebase_initialized = True
        print("Firebase initialized from credentials JSON file.")
    except FileNotFoundError:
        print("Warning: Firebase credentials not found. Falling back to local file serving.")
        firebase_initialized = False
...
@app.post("/input-data")
async def input_data_endpoint(payload: InputDataPayload):
    fastapi_res = await index(prompt(prompt=payload.data))
    return {
        ...
        for scene in result.get("scenes", []):
            ...
            if os.path.exists(mp4_path):
                if firebase_initialized:
                    ...
                    # Return public URL if running on Render, otherwise localhost
                    backend_url = os.getenv("RENDER_EXTERNAL_URL") or os.getenv("BACKEND_URL") or "http://localhost:8000"
                    local_url = f"{backend_url}/{mp4_path}"
                    video_urls.append(local_url)
                    print(f"Firebase not initialized. Serving locally at {local_url}")
            else:
